...
private void updateChart (LineChart chart, LineData data, int color ,float newX, float max,float min){
    ...
    protected void onStart() {
        ...
        WebSocketManager.sendMessage(4, "mpu"); //send message that the activity has switched
    }
    @Override
            ...
            if (action.equals("failToConnect")) {// Handle connection failure
                ...
                runOnUiThread(() -> {
                    if(status.equals("stopSpinning"))
                    {
                        ...
                        isTopSpinning.setText("Not Spinning");  // Set text
                        isTopSpinning.setTextColor(ContextCompat.getColor(MpuActivity.this, R.color.red));  // Set text color
                    }
                    if(status.equals("startSpinning")) {
                        MainClass.top_is_spinning = true;
                        isTopSpinning.setText("Spinning...");  // Set text
                        isTopSpinning.setTextColor(ContextCompat.getColor(MpuActivity.this, R.color.green));  // Set text color
                    }
                });
                ...
                runOnUiThread(() -> {
                    ...
                    updateGraph(time/1000, gyro, 1);
                    updateGraph(time/1000, count, 2);
                });
            }
        } catch (Exception e) {
        e.printStackTrace();
