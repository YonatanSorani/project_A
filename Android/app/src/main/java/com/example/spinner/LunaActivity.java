            ...
            if (action.equals("failToConnect")) {// Handle connection failure
                ...
                runOnUiThread(() -> {
                    if(status.equals("stopSpinning"))
                    {
                        ...
                        isTopSpinning.setText("Not Spinning");  // Set text
                        isTopSpinning.setTextColor(ContextCompat.getColor(LunaActivity.this, R.color.red));  // Set text color
                    }
                    if(status.equals("startSpinning")) {
                        MainClass.top_is_spinning = true;
                        isTopSpinning.setText("Spinning...");  // Set text
                        isTopSpinning.setTextColor(ContextCompat.getColor(LunaActivity.this, R.color.green));  // Set text color
                    }
                });
            }else if (action.equals("data")) {
                float time = (float) jsonResponse.getDouble("time");//time
                float distance = (float) jsonResponse.getDouble("distance");
                float strength = (float) jsonResponse.getDouble("strength");
                float temperature = (float) jsonResponse.getDouble("temperature");//temperature

                // Update the UI with the received direction
                runOnUiThread(() -> {
                    updateGraph( time/1000, distance, 0);
                    updateGraph(time/1000, strength, 1);
                    updateGraph(time/1000, temperature, 2);
                });
            }
        } catch (Exception e) {
            e.printStackTrace();
